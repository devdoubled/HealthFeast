...
const WeightScreen = ({ onNext }) => {
  ...
  const visibleItems = 5;  // Number of items visible at one time

  const renderItem = ({ item }) => (
    <TouchableOpacity
      style={[styles.item, item === selectedWeight && styles.selectedItem, {height: itemHeight}]}
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        style={[styles.list, {height: itemHeight * visibleItems}]}
        showsVerticalScrollIndicator={false}
        snapToAlignment="start"
        snapToInterval={itemHeight}  // Enable snapping behavior
