...
const WeightScreen = ({ onNext }) => {
  ...
  const [selectedWeight, setSelectedWeight] = useState(50); // Initial weight value

  const weightOptions = Array.from({ length: 100 }, (_, index) => `${index + 1} kg`); // Generate weight options from 1 kg to 100 kg
