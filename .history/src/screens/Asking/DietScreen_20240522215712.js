...
const DietScreen = () => {
  ...
  const getDietDetails = (value) => {
    let dietType, image, description, color;
    if (value >= 1 && value < 2) {
    ...
    return { dietType, image, description, color, color_liner };
  };

  const { dietType, image, description, color, color_liner } = getDietDetails(diet);
  return (
        ...
        <View style={[styles.diet_picker_container, { width: width - 60 }]}>
          <View style={styles.diet_picker}>
            <Slider
              ...
              maximumTrackTintColor={color_liner}
            />
          </View>
        </View>
      </View>
