...
const ActivityScreen = () => {
  ...
  const activityDetailOpacity = useRef(new Animated.Value(1)).current;
  const activityDetailPosition = useRef(new Animated.Value(0)).current;

  // Function to animate activity details
  const animateActivityDetail = () => {
    Animated.parallel([
      Animated.timing(activityDetailOpacity, {
        toValue: 0,
        duration: 300,
        useNativeDriver: true,
      }),
      Animated.timing(activityDetailPosition, {
        toValue: -20,
        duration: 300,
        useNativeDriver: true,
      }),
    ]).start(() => {
      // Update activity details and reset animation values
      activityDetailOpacity.setValue(1);
      activityDetailPosition.setValue(0);
    });
  };
  ...
  const handleActivityChange = (value) => {
    setActivity(value);
    animateActivityDetail(); // Animate activity details on change
  };
