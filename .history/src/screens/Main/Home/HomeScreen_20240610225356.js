...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const historyResponse = await apiClient.get(`/ExerciseHistories/personal/date?date=${formatDateNow()}`);
  ...
  const handleCaloriesTrackerPress = () => {
    navigation.navigate("CaloriesTrackerScreen", { userStatistic: userStatistic, exerciseHistory:  });
  };
