  ...
  useEffect(() => {
    const fetchMealData = async (date) => {
      try {
        ...
        const lunchMeal = mealData.meals.filter(meal => meal.mealTime === 2);
        const dinnerMeal = mealData.meals.filter(meal => meal.mealTime === 3);
        const snackMeal = mealData.meals.filter(meal => meal.mealTime === 4);
        const sumBreakfastCalories = breakfastMeal.reduce((totalCalories, meal) => totalCalories + meal.calCalories, 0);
        const sumLunchCalories = lunchMeal.reduce((totalCalories, meal) => totalCalories + meal.calCalories, 0);
        const sumDinnerCalories = dinnerMeal.reduce((totalCalories, meal) => totalCalories + meal.calCalories, 0);
        const sumSnackCalories = snackMeal.reduce((totalCalories, meal) => totalCalories + meal.calCalories, 0);
        setTotalCalories({
          breakfast: sumBreakfastCalories,
          lunch: sumLunchCalories,
          dinner: sumLunchCalories,
          snack: sumSnackCalories
        })
