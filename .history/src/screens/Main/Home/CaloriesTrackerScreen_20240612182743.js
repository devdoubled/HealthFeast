  ...
  useEffect(() => {
    const fetchMealData = async (date) => {
      try {
        ...
        const lunchMeal = mealData.meals.filter(meal => meal.mealTime === 1);
        const dinnerMeal = mealData.meals.filter(meal => meal.mealTime === 1);
        const breakMeal = mealData.meals.filter(meal => meal.mealTime === 1);
