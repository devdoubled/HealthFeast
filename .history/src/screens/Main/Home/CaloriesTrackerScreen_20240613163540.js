...
const CaloriesTrackerScreen = ({ route }) => {
  ...
  const fetchExerciseData = async (date) => {
    try {
      ...
      const historyResponse = await apiClient.get(
        `/ExerciseHistories/personal/date?date=${formattedDate}`
      );
  ...
  const fetchMealData = async (date) => {
    try {
      const formattedDate = formatDateNow(date);
      const mealHistoryResponse = await apiClient.get(
        `/MealHistories/personal/date?date=${formattedDate}`
      );
      const mealData = mealHistoryResponse.data;
      if (mealData) {
        const sumsByMealTime = mealData.meals.reduce((sums, meal) => {
          sums[meal.mealTime] = (sums[meal.mealTime] || 0) + meal.calCalories;
          return sums;
        }, {});

        setTotalCalories({
          breakfast: sumsByMealTime[1] || 0,
          lunch: sumsByMealTime[2] || 0,
          dinner: sumsByMealTime[3] || 0,
          snack: sumsByMealTime[4] || 0,
        });
      }
      ...
      <View style={[styles.tracker_container, { width: width - 32 }]}>
        ...
        <View style={styles.meal_tracker}>
          <View style={styles.tracker}>
            <MealBar label={"Ăn sáng"} value={totalCalories.breakfast} maxValue={roundToDecimal(userStatistic.tdee * 0.25)} />
            <MealBar label={"Ăn trưa"} value={totalCalories.lunch} maxValue={roundToDecimal(userStatistic.tdee * 0.35)} />
          </View>
          <View style={styles.tracker}>
            <MealBar label={"Ăn vặt"} value={totalCalories.snack} maxValue={roundToDecimal(userStatistic.tdee * 0.2)} />
            <MealBar label={"Ăn tối"} value={totalCalories.dinner} maxValue={roundToDecimal(userStatistic.tdee * 0.2)} />
          </View>
        </View>
