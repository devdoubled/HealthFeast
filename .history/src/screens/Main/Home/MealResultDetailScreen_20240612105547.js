...
const MealResultDetailScreen = ({ route }) => {
  ...
  const [modalSuccessVisible, setModalSuVisible] = useState(false);
  ...
  const handleAddMealBreakfast = async () => {
    ...
    try {
      const res =  await apiClient.post('/Meals/database', mealData);
  ...
  const handleAddMealDinner = async () => {
    const mealData = {
      recipeId: meal.recipeId,
      quantity: valueChooseCalories,
      calCalories: valueCalories,
  ...
  const handleAddMealSnack = async () => {
    const mealData = {
      recipeId: meal.recipeId,
      quantity: valueChooseCalories,
      calCalories: valueCalories,
