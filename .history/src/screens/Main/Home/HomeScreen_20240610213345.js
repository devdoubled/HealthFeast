...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchUserStatistic = async () => {
      try {
        const response = await apiClient.get(`/AccountStatistics/${user.accountId}`);
        const userStatisticData = response.data
        setUserStatistic(userStatisticData);
      } catch (error) {
        console.error('Error fetching user statistic:', error);
      }
    };
  ...
  const handlePressExcersize = () => {
    navigation.navigate("ExerciseScreen", {userStatistic: });
  };
  return (
    <ScrollView
