...
const HomeScreen = ({navigation}) => {
  ...
  const handleAddMealPress = (meal) => {
    navigation.navigate("AddMealScreen", {mealScreen: meal});
  }
  return (
    <ScrollView
    ...
    >
      ...
      {/* Daily tracking */}
      <Pressable style={[styles.daily_tracking, { width: width - 32 }]} onPress={handleCaloriesTrackerPress}>
      ...
      <View style={[styles.daily_meal, { width: width - 32 }]}>
        ...
        {meals.map((meal) => (
          <Meal width={width} key={meal.id} meal={meal} handleAddMealPress={handleAddMealPress}/>
        ))}
      </View>
    </ScrollView>
  );
