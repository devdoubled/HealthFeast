...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchExerciseAndMealData = async (date) => {
      try {
        const formattedDate = formatDateNow(date);
  
        const [historyResponse, mealHistoryResponse] = await Promise.all([
          apiClient.get(`/ExerciseHistories/personal/date?date=${formattedDate}`),
          apiClient.get(`/MealHistories/personal/date?date=${formattedDate}`)
        ]);
    ...
    fetchExerciseAndMealData(value);
  }, [value]);
  ...
  const handleAddMealPress = (mealId) => {
    ...
    console.log(roundToDecimal(userStatistic.tdee) - (roundToDecimal(userStatistic.tdee) - mealStatisticHistory.totalCalories + exerciseHistory.totalCalBurned));
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <CircularProgress eaten={roundToDecimal(userStatistic.tdee) - mealStatisticHistory.totalCalories + exerciseHistory.totalCalBurned} target={roundToDecimal(userStatistic.tdee)} />
