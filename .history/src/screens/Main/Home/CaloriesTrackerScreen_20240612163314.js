...
const CaloriesTrackerScreen = ({ route }) => {
  ...
  useEffect(() => {
    const fetchData = async (date) => {
      try {
        // Fetch meal history
        const mealHistoryResponse = await apiClient.get(`/MealHistories/personal/date?date=${formatDateNow(date)}`);
      ...
      <View style={[styles.tracker_container, { width: width - 32 }]}>
        ...
        <View style={styles.explain_container}>
          <Text style={styles.calculate}>
            {roundToDecimal(userStatistic.tdee) - (mealStatisticHistory?.totalCalories || 0) + exerciseHistory.totalCalBurned} = {roundToDecimal(userStatistic.tdee)} - 1256 + {exerciseHistory.totalCalBurned}
          </Text>
