    ...
    const fetchMealData = async (date) => {
      try {
        ...
        if(mealData) {
          ...
          const lunch = mealData.meals.filter(meal => meal.mealTime === 1)
          const dinner = mealData.meals.filter(meal => meal.mealTime === 1)
          const b = mealData.meals.filter(meal => meal.mealTime === 1)
