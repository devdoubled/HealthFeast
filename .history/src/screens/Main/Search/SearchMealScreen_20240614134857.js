...
const SearchMealScreen = ({ navigation, route }) => {
    ...
    try {
      ...
      favoritesData.forEach(item => {
        switch (item.mealTime) {
          case 1:
            organizedData.breakfast.meals.push(item);
            break;
          case 2:
            organizedData.lunch.meals.push(item);
            break;
          case 3:
            organizedData.dinner.meals.push(item);
            break;
          case 4:
            organizedData.snack.meals.push(item);
            break;
          default:
            break;
        }
  ...
  const handleToggleMealSelection = (mealType, favoriteRecipeId) => {
    setMealTimeData(prevMealTimeData => {
      const selectedMeals = prevMealTimeData[mealType].selected;
      const newSelectedMeals = selectedMeals.includes(favoriteRecipeId)
        ? selectedMeals.filter(id => id !== favoriteRecipeId)
        : [...selectedMeals, favoriteRecipeId];
    ...
    <View style={styles.meal_list}>
      {mealData.length > 0 ? (
        mealData.map((meal, index) => {
          const isSelected = selectedMeals.includes(meal.favoriteRecipeId);
          return (
            <Pressable
              key={index}
              onPress={() => handleToggleMealSelection(mealType, meal.favoriteRecipeId)}
              style={[
                styles.meal_item,
