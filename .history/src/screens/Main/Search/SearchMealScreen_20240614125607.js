...
const SearchMealScreen = ({ navigation, route }) => {
  ...
  const fetchFavoriteRecipes = async () => {
    try {
      ...
      const breakfastData = filterFavoritesData.filter(item => item.mealTime === 1)
      const lunchData = filterFavoritesData.filter(item => item.mealTime === 2)
      const dinnerData = filterFavoritesData.filter(item => item.mealTime === 3)
      const snackData = filterFavoritesData.filter(item => item.mealTime === 4)
  ...
  return (
    <View style={styles.container}>
      ...
      >
          ...
          <View style={styles.meal_list}>
            {mealData.map((meal) => {
              ...
              return (
                <Pressable
                  key={meal.id}
                  onPress={() => handleToggleMealSelection(meal.id)}
                  style={[
                    styles.meal_item,
        ...
        <View style={styles.meal_content}>
          <View style={styles.meal_header}>
            <Text style={styles.meal_titie}>Bữa trưa</Text>
            {selectedMeals.length > 0 ? (
              <Ionicons name="trash-outline" size={28} color="#9ABF5A" />
            ) : (
              <Entypo name="dots-three-horizontal" size={28} color="#9ABF5A" />
        ...
        <View style={styles.meal_content}>
          <View style={styles.meal_header}>
            <Text style={styles.meal_titie}>Bữa tối</Text>
            {selectedMeals.length > 0 ? (
              <Ionicons name="trash-outline" size={28} color="#9ABF5A" />
            ) : (
              <Entypo name="dots-three-horizontal" size={28} color="#9ABF5A" />
        ...
        <View style={styles.meal_content}>
          <View style={styles.meal_header}>
            <Text style={styles.meal_titie}>Ăn vặt</Text>
            {selectedMeals.length > 0 ? (
              <Ionicons name="trash-outline" size={28} color="#9ABF5A" />
            ) : (
              <Entypo name="dots-three-horizontal" size={28} color="#9ABF5A" />
