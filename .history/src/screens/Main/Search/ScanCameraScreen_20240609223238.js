...
const ScanCameraScreen = ({ navigation }) => {
  ...
  const takePicture = async () => {
    if (cameraRef.current) {
      ...
      const imageLink = "https://banhmihanoi.net/wp-content/uploads/2020/05/huong-dan-lam-banh-mi-thit-nuong-1.jpg"
      try {
        // Call the separate API function
        const result = await uploadImageForScan();
        console.log(result);
  ...
  const uploadImageForScan = async (imageLink) => {
    try {
      const response = await apiClient.post('https://your-api-endpoint/file', imageLink);
