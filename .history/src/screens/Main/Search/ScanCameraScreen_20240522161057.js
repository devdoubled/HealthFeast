...
const ScanCameraScreen = ({ navigation }) => {
  ...
  const callApiWithImage = async (image) => {
    if (!accessToken) {
      // Handle missing access token (e.g., prompt for login)
    ...
    try {
      const response = await axios.post(
        'https://khang.systems/api/FoodScan/file', // Replace with your actual API endpoint
        formData,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`, // Set Authorization header with token
          },
        }
      );
