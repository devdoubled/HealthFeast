  ...
  const fetchRecipes = async () => {
    try {
      ...
      const transformedRecipes = recipes.map(recipe => ({
        ...
        unit: 'g', // Assuming 'g' (grams) is the unit for recipes
        totalCalories: recipe.totalCalories,
        type: 'recipe', // Indicates this item is a recipe
      }));
      ...
      const transformedIngredients = ingredients.map(ingredient => ({
        ...
        unit: ingredient.countUnit,
        totalCalories: ingredient.calories, // Assuming `calories` represents the total calories for the ingredient
        type: 'ingredient', // Indicates this item is an ingredient
      }));

      // Combine recipes and ingredients into a single array
      const mixedArray = [...transformedRecipes, ...transformedIngredients];

      // Shuffle the mixedArray (optional step to randomize the order)
      shuffleArray(mixedArray);

      setMenuList(mixedArray);
    } catch (error) {
    ...
    <View style={styles.container}>
      ...
      >
        {menuList.map((menuItem) => (
          <View style={styles.menu_item} id={menuItem.id}>
