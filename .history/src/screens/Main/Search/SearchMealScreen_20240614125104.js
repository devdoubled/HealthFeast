...
const SearchMealScreen = ({ navigation, route }) => {
  ...
  const fetchFavoriteRecipes = async () => {
    try {
      ...
      const lunchData = filterFavoritesData.filter(item => item.mealTime === 1)
      const dinnerData = filterFavoritesData.filter(item => item.mealTime === 1)
      const snackData = filterFavoritesData.filter(item => item.mealTime === 1)
