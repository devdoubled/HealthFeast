...
const HistoryExercise = () => {
  ...
  useEffect(() => {
    const fetchExerciseHistory = async () => {
      try {
        ...
        const data = res.data;
        const allExercises = data.map(historyEntry => historyEntry.exercises);
        const allDates = data.map(historyEntry => historyEntry.date);
  ...
  return (
    ...
    >
      {exerciseHistory.map((exerciseGroup, groupIndex) => (
        exerciseGroup.map((ex, exIndex) => (
          <Pressable style={styles.exercise_item} key={`${groupIndex}-${ex.exerciseId}`}>
            ...
            <View style={styles.exercise_text_container}>
              ...
              <Text style={styles.exercise_desc}>Thời gian: {dateHistory[groupIndex]}</Text>
            </View>
          </Pressable>
        ))
      ))}
